...
public class StringUtil {
     ...
     * @throws Exception
     */
    public static String generateAddress(String source) throws Exception {
        String uuid = UUID.randomUUID().toString();
        String addrSrc = source  + uuid + System.currentTimeMillis();
        //两次sha256
        String address = SHAUtil.getSHA256(SHAUtil.getSHA256(addrSrc));
        //地址=Base58(RipeMD160(SHA25(SHA25(有原始数据+调用端公钥+UUID))))
        try {
            address = Base58Util.encode(RipeMDUtil.encodeRipeMD160(address.getBytes(Constants.ENCODING_UTF)));
            return address;
        } catch (Exception e) {
            e.printStackTrace();
            throw new Exception("generate address error:" + e.getMessage());
        }
    }
     ...
     * @return 新字符串
     */
    public static String replaceBigChar(String src){
        StringBuilder result = new StringBuilder();
        char[] charArray = src.toCharArray();
        for (char aCharArray : charArray) {
            if (aCharArray >= 'A' && aCharArray <= 'Z') {
                result.append("_").append(aCharArray);
            } else {
                result.append(aCharArray);
            }
        }
