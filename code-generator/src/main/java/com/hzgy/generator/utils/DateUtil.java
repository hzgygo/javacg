...
public class DateUtil {
	...
	public static String dateFromat(Date date,int minute){
		...
		int hour = minute / 60;
		int min = minute % 60;
		dateFormat = String.valueOf(year) + (month > 9?String.valueOf(month):"0" + String.valueOf(month)) + (day > 9?String.valueOf(day):"0" + String.valueOf(day)) + " " + (hour > 9?String.valueOf(hour):"0" + String.valueOf(hour)) + (min > 9?String.valueOf(min):"0" + String.valueOf(min)) + "00";
		return dateFormat;
	}
	...
	public static List<Date> betweenDates(String beginDate,String endDate){
		List<Date> listDate = new ArrayList<Date>();
		try{
			SimpleDateFormat sdf = new SimpleDateFormat(DateUtil.ISO_EXPANDED_DATE_FORMAT);
	...
	public static List<String> betweenStringDates(String beginDate,String endDate){
		List<String> listDate = new ArrayList<String>();
		try{
			SimpleDateFormat sdf = new SimpleDateFormat(DateUtil.ISO_EXPANDED_DATE_FORMAT);
			Date begin = sdf.parse(beginDate);
			Date end = sdf.parse(endDate);
			listDate.add(DateUtil.dateToString(begin,DateUtil.ISO_EXPANDED_DATE_FORMAT));
			Calendar calBegin = Calendar.getInstance();
			// 使用给定的 Date 设置此 Calendar 的时间
			calBegin.setTime(begin);
			Calendar calEnd = Calendar.getInstance();
			...
			while(end.after(calBegin.getTime())){
				// 根据日历的规则，为给定的日历字段添加或减去指定的时间量
				calBegin.add(Calendar.DAY_OF_MONTH,1);
				listDate.add(DateUtil.dateToString(calBegin.getTime(),DateUtil.ISO_EXPANDED_DATE_FORMAT));
			}
		}
		catch(ParseException ex){
			ex.printStackTrace();
