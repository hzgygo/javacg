$(document).ready(function(e) {
	if (!url){
		url = "<#noparse>${base}</#noparse>/ajax/getdata/${viewConfig.entityName}/paginated.json";
	}
	if (!postData){
		postData = {c:'${viewConfig.authCode?c}'};
	}
	else{
		postData = $.extend(true, postData,{c:'${viewConfig.authCode?c}'});
	}
    jQuery("#userList").jqGrid({
    	treeGrid: true,
    	<#assign expandColumn="name"/>
    	<#if viewFieldConfigs?? >
        <#list viewFieldConfigs as viewFieldConfig>
        <#if viewFieldConfig.isTreeListField?? >
        <#if viewFieldConfig.isTreeListField == 1 >
        <#assign expandColumn="${viewFieldConfig.fieldCode!''}"/>
        </#if>
        </#if>
        </#list>
        </#if>	
  		ExpandColumn : "${expandColumn?uncap_first}",     
