...
var pageRows = "<#noparse>${rows}</#noparse>";
var currentPage = "<#noparse>${page}</#noparse>";
var gridUrl = "<#noparse>${base}</#noparse>/ajax/getdata/${viewConfig.entityName}/${viewConfig.dataMethodName}.json";
var gridPostData = {c:'${viewConfig.authCode?c}'};
function loadComplete(gdatas){
	...
	for(var i=0;i < rows.length;i++){
		...
		content += "<a href='javascript:void(0)' data-init='" + $.toJSON(rows[i]) + "' class='shortcut_${subViewConfig.dataMethodName?uncap_first}'";
...
<table width="99%" border="0" cellspacing="1" cellpadding="0" class="skinMain">
	<#list subViewConfigs as subViewConfig>
	<#if subViewConfig.parentViewPosition == 4>
	<tr>
		<td width="100%" id="search">
		</td>
	</tr>	
	</#if>
	</#list>			
	<tr>
		<td width="100%">
			<table cellpadding="0" cellspacing="0" border="0" width="99%">
				<tr>
					<td width="93%" height="650px" valign="top">
						<table id="${viewConfig.entityName}List">
							<tr>
								<td>&nbsp;</td>
							</tr>
						</table>
